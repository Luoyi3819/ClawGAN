...
def _handle_path(path, sess, low_profile=False):
    if path.endswith('.npz'):
        ...
        if low_profile:
            ...
            x = np.array([cv2.imread(str(fn)).astype(np.float32) for fn in files])
...
if __name__ == "__main__":
    path = ["../test_data/infrared",'../test_data/visible']  ##test fid 
    fid_value = calculate_fid_given_paths(path, inception_path = None, low_profile=False)
    print("FID: ", fid_value)
